...
def prepare_iam_model(mcd):
    ...
    mcd.add_service(node_id=func_arn, node_name='prod-lambda-name', arn=func_arn, node_enum=AWS.lambda_function)

    role_arn = 'arn:aws:iam::123456789012:role/prod-lambda-name'
    mcd.add_service(node_id=role_arn, node_name='role-lambda-name', arn=role_arn, node_enum=AWS.iam_role)

    cw_policy_arn = "arn:aws:iam::123456789012:policy/prod-cloudwatch-policy"
    mcd.add_service(node_id=cw_policy_arn, node_name='prod-cloudwatch-policy', arn=cw_policy_arn, node_enum=AWS.iam_policy)

    s3_policy_arn = "arn:aws:iam::123456789012:policy/prod-s3-policy"
    mcd.add_service(node_id=s3_policy_arn, node_name='prod-s3-policy', arn=s3_policy_arn, node_enum=AWS.iam_policy)
